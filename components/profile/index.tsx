...
import { ProfileData, ProfileStats as ProfileStatsType, Activities } from "./types"
import ProfileTabs from "./profile-tabs"
import { ProfileSkeleton } from "../post/skeletons"
...
const Profile = ({ data, initialData, profileId, error, correctSlug }: PostProps) => {
    const router = useRouter()
    const [statsLoaded, setStatsLoaded] = useState(false)
    const [activitiesLoaded, setActivitiesLoaded] = useState(false)
    const [statsData, setStatsData] = useState<ProfileStatsType | undefined>(undefined)
    const [activitiesData, setActivitiesData] = useState<Activities | undefined>(undefined)
    ...
    useEffect(() => {
        if (profileData?.profile && profileId && !statsLoaded && !data) {
            const loadStats = async () => {
                try {
                    const response = await axios.get(`/api/profile/${profileId}/stats`)
    ...
    useEffect(() => {
        if (profileData?.profile && profileId && !activitiesLoaded && !data) {
            const loadActivities = async () => {
                try {
                    const response = await axios.get(`/api/profile/${profileId}/activities`)
