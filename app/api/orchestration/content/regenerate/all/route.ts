...
export async function POST() {
    try {
        ...
        console.log("💰 TOTAL AI REGENERATION COST SUMMARY:")
        console.log(
        ...
        return NextResponse.json({
            success: true,
            message: "Content regeneration completed successfully",
            data: {
                main: mainContent,
                ...
                linkedinImage: sharedImage,
                xImage: sharedImage,
                metaImage: sharedImage,
            },
        })
    } catch (error) {
        console.error("Error in content regeneration:", error)
        return NextResponse.json(
            { success: false, message: "Failed to regenerate content" },
            { status: 500 }
        )
    }
}
